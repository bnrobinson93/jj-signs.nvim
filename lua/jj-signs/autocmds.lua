...
-- Call the coroutine body inline (not pipeline.refresh): throttle_async already
-- runs this inside a coroutine it owns, and the body's `await`s must suspend THAT
-- coroutine so a burst collapses into one trailing refresh (see
-- pipeline._refresh_impl).
local throttled_refresh = async.throttle_async(
  function(bufnr) require("jj-signs.pipeline")._refresh_impl(bufnr) end,
