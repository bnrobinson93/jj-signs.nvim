...
![Live edits in a jj change: add/change/delete signs, inline word-diff, undo/redo](assets/signs.gif)
...
![A rebase conflict, materialized and shown with ours/base/theirs region tints](assets/conflict.gif)

Beyond the signs, the interactive actions: intra-line **word-diff**, **`nav_hunk`**
between hunks, **`preview_hunk_inline`** (removed lines shown dimmed in place),
**`change_base @--`** to widen the comparison one change further back, and the
**blame** popup and full-file blame split.

![nav_hunk, inline hunk preview, change_base, and blame on a wip change](assets/features.gif)

> All three GIFs are generated by `scripts/make-demo.sh` (asciinema + agg) — it records
> only the terminal grid, so no machine details are captured.
